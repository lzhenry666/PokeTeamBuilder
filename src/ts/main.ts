...
async function carregarListaDePokemons() {
  try {
    ...
    btnChoose.addEventListener("click", async (event) => {
      ...
      console.log(
        `🚀 ~ file: main.ts:109 ~ btnChoose.addEventListener ~ pokemonName:`,
        pokemonName
      );
...
async function buscarPreEvolucao(
  pokemonName: string
): Promise<PreEvolucaoResultado> {
  try {
    ...
    const evolutionData = await axios.get(
      `https://pokeapi.co/api/v2/pokemon/${evolutionChain_pre_name}`
    );
...
function addTooltipEvents() {
  ...
  imgs.forEach((img: HTMLImageElement) => {
    img.addEventListener("mouseover", (event: MouseEvent) => {
  ...
  try {
    ...
    for (let slot of slots) {
      if (
        ...
        if (resultado) {
          const { prevEvolution_img, evolutionChain_pre_name } = resultado as {
            prevEvolution_img: string;
            evolutionChain_pre_name: string;
          };

          if (prevEvolution_img) {
            const preEvolutionElement = slot.querySelector(
              ".pre-pokemon"
            ) as HTMLElement;
            (
              preEvolutionElement as HTMLImageElement
            ).src = `${prevEvolution_img}`;
          }

          if (evolutionChain_pre_name) {
            const span = slot.querySelector(
              ".sub-header span:first-child"
            ) as HTMLElement;
            const span_ = slot.querySelector(
              ".sub-header span:last-child"
            ) as HTMLElement;

            //  console.log(`🚀 ~ file: main.ts:211 ~ escolherPokemon ~ span:`, span);
            (span as HTMLSpanElement).innerHTML += `${
              evolutionChain_pre_name.slice(0, 1).toUpperCase() +
        ...
        function addImagesToContainer(
          ...
          typesArray.forEach((type) => {
            ...
            const typeData = type_info.find(
              (info) =>
                info.type == type.slice(0, 1).toUpperCase() + type.slice(1)
            );
        ...
        response.data.types.forEach((typeObj: any) => {
          const typeName = typeObj.type.name;
          const typeData = type_info.find(
            (info) => info.type.toLowerCase() === typeName
          );
          ...
          if (typeData) {
            ...
            const advantageContainer = slot.querySelector(
              ".advantage-container"
            );
            ...
            const resistanceContainer = slot.querySelector(
        ...
        const typeNames: string[] = response.data.types.map(
        ...
        slot.querySelector(
          ".pokemonEspecialAttack"
        )!.innerHTML = `${pokemonEspecialAttack}`;
        slot.querySelector(
          ".pokemonEspecialDefense"
        )!.innerHTML = `${pokemonEspecialDefense}`;
...
function resetarTime() {
  ...
  for (let slot of slots) {
    ...
    const pokemonImgElement_ = slot.querySelector(
      ".pre-pokemon"
    ) as HTMLImageElement;
    pokemonImgElement_.src = "pokebolaa.gif";
    pokemonImgElement_.alt = "";
    const type_img = slot.querySelectorAll(
      ".type-img"
    ) as NodeListOf<HTMLImageElement>;
    ...
    (
      slot.querySelector(".pokemonEspecialAttack") as HTMLDivElement
    ).innerHTML = ``;
    (
      slot.querySelector(".pokemonEspecialDefense") as HTMLDivElement
    ).innerHTML = ``;
    (slot.querySelector(".pokemonSpeed") as HTMLDivElement).innerHTML = ``;
    (
      slot.querySelector(".pre-pokemon") as HTMLDivElement
    ).style.background = ``;
    slot.style.background = ``;
    slot.querySelector(".attack-content")!.innerHTML = ``;
  }
}
function gerarTextoPokemonShowdown(pokemon: any) {
...
function copiarTextoParaClipboard(texto: string) {

...
async function buscarPreEvolucao(pokemonName) {
  try {
    // Primeiro, obtenha o ID da cadeia de evolução
    const speciesResponse = await axios.get(`https://pokeapi.co/api/v2/pokemon-species/${pokemonName}/`);
    const evolutionChainUrl = speciesResponse.data.evolution_chain.url;

    // Em seguida, obtenha a cadeia de evolução
    const evolutionResponse = await axios.get(evolutionChainUrl);
    const chain = evolutionResponse.data.chain;

    // Navegue pela cadeia para encontrar a pré-evolução
    let preEvolutionName = null;
    let current = chain;
    while (current && current.species.name !== pokemonName) {
      preEvolutionName = current.species.name;
      current = current.evolves_to[0];
    }

    if (preEvolutionName) {
      // Se encontrarmos uma pré-evolução, obtenha a imagem dela
      const preEvolutionResponse = await axios.get(`https://pokeapi.co/api/v2/pokemon/${preEvolutionName}/`);
      return preEvolutionResponse.data.sprites.front_default;
    } else {
      return null;
    }
  } catch (error) {
   ...
   var  pokemonEspecialDefense = `ESP-DEF:${response.data["stats"][4]["base_stat"]}`;
    ...
    for (let slot of slots) {
      ...
      if (!slot.querySelector('.pokename')!.innerHTML) {
        const preEvolutionImgUrl = await buscarPreEvolucao(pokemonName);
    if (preEvolutionImgUrl) {
      slot.querySelector('.card-avatar.pre-evolution').style.backgroundUrl = preEvolutionImgUrl;
    }

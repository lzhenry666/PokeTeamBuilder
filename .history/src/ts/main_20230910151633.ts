...
async function buscarPreEvolucao(pokemonName : string) {
    try {
      ...
      if (evolutionChain_pre_name === null || evolutionChain_pre_name === undefined || prevEvolution_img === null || prevEvolution_img === undefined) {
        return "egg.png";
      }else{
   ...
   var  pokemonEspecialDefense = `ESP-DEF:${response.data["stats"][4]["base_stat"]}`;
    ...
    for (let slot of slots) {
      ...
      if (slot.querySelector('.pokename')!.innerHTML === '...' || slot.querySelector('.pokename')!.innerHTML === '') {
        const {prevEvolution_img, evolutionC hain_pre_name} = await buscarPreEvolucao(response.data.id);

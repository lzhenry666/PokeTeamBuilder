  ...
  try {
   ...
   var  pokemonEspecialDefense = `ESP-DEF:${response.data["stats"][4]["base_stat"]}`;
    ...
    if (preEvolutionImgUrl) {
      ...
      (preEvolutionElement as HTMLImageElement).src = `${preEvolutionImgUrl}`;
...
function resetarTime() {
  for (let slot of slots) {
      ...
      const pokemonImgElement_ = slot.querySelector('.pokemon-img') as HTMLImageElement;
      pokemonImgElement_.src = '';

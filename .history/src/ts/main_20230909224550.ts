...
async function buscarPreEvolucao(pokemonName : string) {
  try {
    ...
    if (preEvolutionName) {
      ...
      const preEvolutionResponse = await axios.get(`https://pokeapi.co/api/v2/evolution-chain/${preEvolutionName}/`);
   ...
   var  pokemonEspecialDefense = `ESP-DEF:${response.data["stats"][4]["base_stat"]}`;
    ...
    for (let slot of slots) {
      ...
      if (!slot.querySelector('.pokename')!.innerHTML) {
        const preEvolutionImgUrl = await buscarPreEvolucao(response.id);

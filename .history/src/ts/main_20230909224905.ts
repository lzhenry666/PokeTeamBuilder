...
async function buscarPreEvolucao(pokemonName : string) {
  try {
      ...
      const preEvolutionResponse = await axios.get(`https://pokeapi.co/api/v2/evolution-chain/${pokemonName}/`);
      console.log(`🚀 ~ file: main.ts:135 ~ buscarPreEvolucao ~ preEvolutionResponse:`, preEvolutionResponse);
   ...
   var  pokemonEspecialDefense = `ESP-DEF:${response.data["stats"][4]["base_stat"]}`;
    ...
    for (let slot of slots) {
      ...
      if (!slot.querySelector('.pokename')!.innerHTML) {
        console.log("Elemento para o nome:", response.id);

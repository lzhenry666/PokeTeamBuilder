...
async function carregarListaDePokemons() {
  ...
  async function preencherSlotComPokemon(slot, dadosPokemon) {
    const { name, weight, height, types, moves, stats, sprites } = dadosPokemon;
    const pokemonName = name.charAt(0).toUpperCase() + name.slice(1);
    const pokemonWeight = `${weight} Kg`;
    const pokemonHeight = `${height}0 Cm`;
    const pokemonType = types.map(t => `TYPE:${t.type.name}`).join(' ');
    const pokemonHp = `HP:${stats[0].base_stat}`;
    const pokemonAttack = `ATK:${stats[1].base_stat}`;
    // ... (preencha as outras estatísticas aqui)
    ...
    slot.querySelector('.attack-content').innerHTML = ` ${randomMoves}`;
    slot.querySelector('.pokename').innerHTML = ` ${pokemonName}`;
    slot.querySelector('.pokemonWeight').innerHTML = `${pokemonWeight}`;
  ...
  async function escolherPokemon(event) {
    event.preventDefault();
    // ... (seu código para obter os dados do Pokémon)
    for (let slot of slots) {
      if (!slot.querySelector('.pokename').innerHTML) {
        await preencherSlotComPokemon(slot, response.data);
        break;
      }
    }
    // ... (resto do seu código)

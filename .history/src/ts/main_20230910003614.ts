...
async function escolherPokemon(randomIndex: number, pokemonName?: string)  {
  ...
  try {
   ...
   var  pokemonEspecialDefense = `ESP-DEF:${response.data["stats"][4]["base_stat"]}`;
    ...
    let timeCompleto = 0;
      pokemonName? pokemonName.charAt(0).toUpperCase() + pokemonName.slice(1) : response.data.name.charAt(0).toUpperCase() + response.data.name.slice(1);
      console.log(`🚀 ~ file: main.ts:186 ~ escolherPokemon ~ pokemonName:`, response.data.name);

...
async function carregarListaDePokemons() {
  try {
    ...
    btnChoose.addEventListener("click", e => escolherPokemon);
...
async function escolherPokemon(randomIndex: number, event : Event) {
...
async function compartilharTime() {
 ...
 // Função para gerar um Pokémon aleatório
  // Função para gerar um Pokémon aleatório
  async function gerarPokemonAleatorio(event : Event) {
    const randomIndex = Math.floor(Math.random() * 151) + 1;
    event.preventDefault();

    for (let slot of slots) {
      if (!slot.querySelector('span')!.innerText) {
        await escolherPokemon(randomIndex, event);
        break;
      }
    }
  }
